...
class GalacticWidget(QWidget):
    ...
    def mouseMoveEvent(self, event):
        ...
        for item in reversed(self.interactive_items):
            if item['rect'].contains(painter_pos) and item['is_lit']:
    ...
    def _draw_spiral_path(self, painter):
        ...
        dim_color = QColor(80, 80, 80, 80)

        for i in range(len(self.spiral_points) - 1):
            ...
            is_lit = self.lamp_brightness >= target_course['light_required']
    ...
    def _draw_planets_and_moons(self, painter):
        ...
        for course in self.courses:
            is_lit = self.lamp_brightness >= course['light_required']

            # Determine colors based on lit status
            planet_base_color = QColor("#a29bfe") if is_lit else QColor(100, 100, 110)
            social_base_color = QColor("#55efc4") if is_lit else QColor(80, 120, 100)
            mentor_base_color = QColor("#ffeaa7") if is_lit else QColor(120, 120, 100)
            icon_base_color = QColor("#006266") if is_lit else QColor(50, 70, 70)

            point_index = int(course["progress"] * (len(self.spiral_points) - 1))
            planet_pos = self.spiral_points[point_index]

            is_hovered = (course['id'] == self.hovered_item_id and is_lit)
            planet_radius = base_planet_radius * 1.5 if is_hovered else base_planet_radius

            painter.setBrush(planet_base_color)
            painter.setPen(Qt.PenStyle.NoPen)
            painter.drawEllipse(planet_pos, planet_radius, planet_radius)

            status = course.get('completion_status', 'not_started')
            if is_lit and (status == 'in_progress' or status == 'completed'):
            ...
            self.interactive_items.append({"id": course['id'], "name": course['name'], "rect": planet_rect, "type": "course_planet", "is_lit": is_lit})

            # --- Moons ---
            social_id = f"social_{course['id']}"
            is_social_hovered = (social_id == self.hovered_item_id and is_lit)
            ...
            painter.setBrush(social_base_color)
            painter.drawEllipse(moon1_pos, social_radius, social_radius)

            icon_pen = QPen(icon_base_color)
            ...
            self.interactive_items.append({"id": social_id, "rect": social_rect, "type": "moon_social", "is_lit": is_lit})

            mentor_id = f"mentor_{course['id']}"
            is_mentor_hovered = (mentor_id == self.hovered_item_id and is_lit)
            ...
            painter.setBrush(mentor_base_color)
            painter.setPen(Qt.PenStyle.NoPen)
            ...
            self.interactive_items.append({"id": mentor_id, "rect": star.boundingRect(), "type": "moon_mentor", "is_lit": is_lit})
